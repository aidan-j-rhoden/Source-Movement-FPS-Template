...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton or Input.is_action_just_pressed("ui_accept"): # Disable this later, and simply let logic check for the current mouse mode.
...
func die() -> void: ## Kill the player, for physics
	...
	ragdoll.reparent(get_node("/root/Map01/ragdolls"), true) # Reparent the ragdoll to a ragdoll-only node in the map.
